...
We consider these tools a legacy, but will maintain them through this version.
...
1. Initialize and Dispose of the Input system\
   To do this, you need to call API.Input.Client.RunFrame on start. In UnityEditor, you should also call `Application.OpenURL($"steam://forceinputappid/{SteamSettings.ApplicationId}");` to force the Steam client to use your app's input settings\
   And when you're done, call `Application.OpenURL("steam://forceinputappid/0");`
...
For a functional coded example, simply review the source code of SteamInputManager.cs
...
We created InputAction as a type of Game Event. So this means you can register and listen for changes on each input action from anywhere. We have tools like the Input Action Event that let you set this up easily in Unity Editor's Inspector.
