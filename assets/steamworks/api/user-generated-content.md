...
{% hint style="info" %}
The [UGC Query Manager](../unity-engine/components/ugc-query-manager.md) can make working with quires much simpler.
{% endhint %}
...
To learn more about [callbacks](../../../company/concepts/development/callbacks.md) please read the related article.
...
The easiest way to handle a UGC / Workshop browser in game is to use the [UGC Query Manager](../unity-engine/components/ugc-query-manager.md). The manager uses the same features present in the interface so it can be done manually.
...
You can see how we implemented paging in the [UGC Query Manager](../unity-engine/components/ugc-query-manager.md) if you wished to do so your self or simply to better understand the system.
