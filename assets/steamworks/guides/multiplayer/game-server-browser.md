...
Put simply server discovery. Steam Game Servers can be configured to list on Steam's Game Server Browser. This can then be used in or out of game to discover game sessions to play or spectate. See the [Game Server Browser Manager](../../for-unity-game-engine/components/game-server-browser-manager.md) document for more information
...
To configure a Steam Game Server browser simply set the desired settings in your [Steam Settings](../../for-unity-game-engine/scriptable-objects/steam-settings/#steamsettings.server) object and build a server build. In particular take note of the [Game Server](../../for-unity-game-engine/scriptable-objects/steam-settings/game-server.md) settings.
