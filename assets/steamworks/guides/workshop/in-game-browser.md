...
{% hint style="info" %}
How you set up the UI is up to you, the [UGC Query Manager](../../for-unity-game-engine/components/ugc-query-manager.md) manages the query and its results for you, its up to you to display those to the user how you see fit.
{% endhint %}
...
The first thing you will want to do is add a [UGC Query Manager](../../for-unity-game-engine/components/ugc-query-manager.md) to a game object related to your browser UI. This component provides the events your UI will listen on to update its display and the methods and values your UI will interact with to drive the query

### Step 2

Create a UI controller that can listen on the Results Changed event of the UGC Query Manager and update your UI accordingly. That is write a new script to manage your browser UI and set it up such that it can listen on the [Results Updated](../../for-unity-game-engine/components/ugc-query-manager.md#events) event of the manager
...
The above assumes you have a reference to your [query manager](../../for-unity-game-engine/components/ugc-query-manager.md) in an attribute named `manager`&#x20;
