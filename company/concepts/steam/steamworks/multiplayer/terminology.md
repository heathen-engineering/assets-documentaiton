...
A lobby however is not a server, browsing for lobbies is not a way to make a server list. Server discovery is a concept which Steam handles please see the [Steam Game Server](game-server-browser.md) and [Game Server Browser](../../../../../assets/steamworks/unity-engine/components/game-server-browser-manager.md) for more information.
