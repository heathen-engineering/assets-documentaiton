...
Steam Deck is not a special use case it is simply a Linux machine with no keyboard or mouse but does have a controller present. You can use native Unity Input to handle controller support or if you prefer [Steam Input](../../../heathens-steamworks-complete/unity/sample-scenes/input.md) is an option though not required.
