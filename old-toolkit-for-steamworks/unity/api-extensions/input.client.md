...
Adds an input action to the list of tracted input actions, if your using a [SteamSettings ](../objects/classes/steam-settings/)object this will be done for you for all InputActions defined in the [SteamSettings](../objects/classes/steam-settings/). Once an action is added it can be updated by the API and tools such as the [SteamInputManager ](../objects/components/steam-input-manager.md)can be used to track changes.
...
Returns the input type (device model) for the specified controller. This tells you if a given controller is a Steam controller, XBox 360 controller, PS4 controller, etc. For more details, see Steam's [Supported Controller Database](https://support.steampowered.com/kb_article.php?ref=5199-TOKV-4426).
